# setupForRun703DailyIBMData2014.R  11/23/2013
#
...
ibmConstantsCRRUsed = function() {
  ...
  K           = 185          # call option strike price
  optionPrice = 10.65        # option price
  nFlips      = 44           # number of rebalancing times.
...
#
  percentCRR = 0        # (1+percentage)*CRR is initial capital
  # Check if using input volatility rather than empirical volatility
  useInputVolatility = FALSE
  ...
  constants  = c(nPaths=nPaths,TimeToExpiration=TimeToExpiration,
                 ...
                 useInputVolatility=useInputVolatility,
...
computeGoodUDRangeForRut = function(priceBaseRut,u,d,nFlips,R,rho,nPtsU,nPtsD,
  ...
  while( (count <= length(divisorVector)) && (! goodRange)) {
    ...
    # Make sure uStart doesn't drop below rho.
    # We need the no arbitrage condition (1+r) < u
    uStart = max(v[1],rho+.0005)
    uEnd   = v[2]
    dStart = v[3]
    dEnd   = v[4]
    # First time through, save uStart,uEnd,dStart,dEnd
    if (count == 1) {
      uStartConstant=uStart
      uEndConstant=uEnd
      dStartConstant=dStart
      dEndConstant=dEnd}
  # If uStar being held constant, assign initial values to uStart,uEnd
  if (keepUStarConstant) {
    uStart=uStartConstant
    uEnd=uEndConstant
  }
...
#  drift     = constants['drift']
  ...
  pathMatrix = newPathsFromNewIBMData()
  ibm        = getActualFutureIBMStockPricePath()
  #
  percentCRR = constants['percentCRR']
  #############################################################
  ## Create CRR market contour udMatrix1 - see sigma1Contour.R
  ##
  volatilityAnnual = constants['volatilityAnnual']
  priceBaseCRR   = optionPrice
  initialCapital = unname(priceBaseCRR * (1 + percentCRR))
  useInputVolatility = constants['useInputVolatility']
  if (useInputVolatility) {
...
#  cat('\n\n\t Base CRR price',priceBaseCRR,'\t uBase,dBase',u,d,'\n')
  ...
  cat('\n useInputVolatility', useInputVolatility, 'volatility',volatility)
  cat("\n\n")
  #
